...
from net_base import get_public_ip, get_local_ip
import builder
import client
import game
...
from ui.element.background import Button_Timer
...
def settings_menu():
    ...
    t = Input((100, 0), message=SAVE.get_data('username'), color=(0, 0, 0), fgcolor=(255, 255, 255), length=50, scroll=True)
    ...
    i = Input(
        (200, 0), message=str(SAVE.get_data('port')), color=(0, 0, 0), fgcolor=(255, 255, 255), 
...
def join_game_menu(name, ip):
    ...
    icon = pg.image.load('img/icons/trash_icon2.png').convert_alpha()
    b = Button.image_button(
        icon, padding=(5, 5), color1=(0, 0, 0, 0), color2=(128, 0, 0), border_radius=5,
